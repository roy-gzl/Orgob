...
Destination folders are configurable in `config.yml`.
...
Weekly summaries use Daily Notes. Monthly summaries use weekly summaries.
...
Yes.  
Set `ai.enabled: false` in `config.yml` to run rule-based automation only.
